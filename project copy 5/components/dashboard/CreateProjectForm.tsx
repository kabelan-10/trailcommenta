...
export default function CreateProjectForm({
  ...
  const handleGenerateExplain = async (link: string) => {
    ...
    try {
      ...
      console.log("------- EXPLAIN response:", response.data!.data.audiance);
      const audiance = response.data!.data.audiance;
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.problem.trim()) {
      newErrors.problem = "Person role is required";
    }

    if (!formData.solution.trim()) {
      newErrors.solution = "Person story is required";
    }
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              type="button"
              onClick={() => handleGenerateExplain(formData.product_link)}
